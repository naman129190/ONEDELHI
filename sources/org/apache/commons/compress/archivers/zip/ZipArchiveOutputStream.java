...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
    ...
    private byte[] createCentralFileHeader(ZipArchiveEntry zipArchiveEntry) throws IOException {
        ...
        boolean z = hasZip64Extra(zipArchiveEntry) || zipArchiveEntry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC || zipArchiveEntry.getSize() >= ZipConstants.ZIP64_MAGIC || longValue >= ZipConstants.ZIP64_MAGIC;
    ...
    private byte[] createCentralFileHeader(ZipArchiveEntry zipArchiveEntry, ByteBuffer byteBuffer, long j, boolean z) throws IOException {
        ...
        ZipLong.putLong(Math.min(j, ZipConstants.ZIP64_MAGIC), bArr, 42);
    ...
    private boolean shouldAddZip64Extra(ZipArchiveEntry zipArchiveEntry, Zip64Mode zip64Mode2) {
        return zip64Mode2 == Zip64Mode.Always || zipArchiveEntry.getSize() >= ZipConstants.ZIP64_MAGIC || zipArchiveEntry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC || !(zipArchiveEntry.getSize() != -1 || this.raf == null || zip64Mode2 == Zip64Mode.Never);
    }
    ...
    private void validateSizeInformation(Zip64Mode zip64Mode2) throws ZipException {
        ...
        if ((this.entry.entry.getSize() >= ZipConstants.ZIP64_MAGIC || this.entry.entry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC) && zip64Mode2 == Zip64Mode.Never) {
            throw new Zip64RequiredException(Zip64RequiredException.getEntryTooBigMessage(this.entry.entry));
        }
    }
    ...
    public void writeCentralDirectoryEnd() throws IOException {
        ...
        } else if (this.cdOffset <= ZipConstants.ZIP64_MAGIC || this.zip64Mode != Zip64Mode.Never) {
            byte[] bytes = ZipShort.getBytes(Math.min(size, 65535));
            writeCounted(bytes);
            writeCounted(bytes);
            writeCounted(ZipLong.getBytes(Math.min(this.cdLength, ZipConstants.ZIP64_MAGIC)));
            writeCounted(ZipLong.getBytes(Math.min(this.cdOffset, ZipConstants.ZIP64_MAGIC)));
    ...
    public void writeZip64CentralDirectory() throws IOException {
        if (this.zip64Mode != Zip64Mode.Never) {
            if (!this.hasUsedZip64 && (this.cdOffset >= ZipConstants.ZIP64_MAGIC || this.cdLength >= ZipConstants.ZIP64_MAGIC || this.entries.size() >= 65535)) {
                this.hasUsedZip64 = true;
            }
