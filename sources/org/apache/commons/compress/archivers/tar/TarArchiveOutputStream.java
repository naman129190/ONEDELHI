...
public class TarArchiveOutputStream extends ArchiveOutputStream {
    ...
    private void addPaxHeadersForBigNumbers(Map<String, String> map, TarArchiveEntry tarArchiveEntry) {
        ...
        addPaxHeaderForBigNumber(map, "gid", tarArchiveEntry.getLongGroupId(), 2097151);
        addPaxHeaderForBigNumber(map, "mtime", tarArchiveEntry.getModTime().getTime() / 1000, TarConstants.MAXSIZE);
        addPaxHeaderForBigNumber(map, "uid", tarArchiveEntry.getLongUserId(), 2097151);
        Map<String, String> map2 = map;
        addPaxHeaderForBigNumber(map2, "SCHILY.devmajor", (long) tarArchiveEntry.getDevMajor(), 2097151);
        addPaxHeaderForBigNumber(map2, "SCHILY.devminor", (long) tarArchiveEntry.getDevMinor(), 2097151);
        failForBigNumber("mode", (long) tarArchiveEntry.getMode(), 2097151);
    }
    ...
    private void failForBigNumbers(TarArchiveEntry tarArchiveEntry) {
        ...
        failForBigNumberWithPosixMessage("group id", tarArchiveEntry.getLongGroupId(), 2097151);
        failForBigNumber("last modification time", tarArchiveEntry.getModTime().getTime() / 1000, TarConstants.MAXSIZE);
        failForBigNumber("user id", tarArchiveEntry.getLongUserId(), 2097151);
        failForBigNumber("mode", (long) tarArchiveEntry.getMode(), 2097151);
        failForBigNumber("major device number", (long) tarArchiveEntry.getDevMajor(), 2097151);
        failForBigNumber("minor device number", (long) tarArchiveEntry.getDevMinor(), 2097151);
    }
    ...
    private String stripTo7Bits(String str) {
        ...
        for (int i = 0; i < length; i++) {
            char charAt = (char) (str.charAt(i) & oo2.f33151b);
    ...
    public void writePaxHeaders(TarArchiveEntry tarArchiveEntry, String str, Map<String, String> map) throws IOException {
        ...
        for (Map.Entry next : map.entrySet()) {
            ...
            String str5 = length + " " + str3 + a13.f25713a + str4 + "\n";
            int length2 = str5.getBytes("UTF-8").length;
            while (length != length2) {
                str5 = length2 + " " + str3 + a13.f25713a + str4 + "\n";
                int i = length2;
